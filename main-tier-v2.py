...
# ── NUEVO: Tier 1 Config ──
STRATEGY_CONFIG = {
    "largo_plazo": {"max_position": 0.05, "max_sector": 0.20, "min_cash": 0.05},
    "swing": {"max_position": 0.03, "max_sector": 0.15, "min_cash": 0.10},
    "crisis": {"max_position": 0.08, "max_sector": 0.25, "min_cash": 0.00},
}

REGIME_WEIGHTS = {
    "bull":    {"bq": 0.60, "val": 0.30, "tim": 0.10},
    "bear":    {"bq": 0.35, "val": 0.15, "tim": 0.50},
    "panic":   {"bq": 0.30, "val": 0.20, "tim": 0.50},
    "neutral": {"bq": 0.50, "val": 0.30, "tim": 0.20},
}
...
class TickerResult:
    ticker: str = ""
    bq: MetricResult = field(default_factory=MetricResult)
    val: MetricResult = field(default_factory=MetricResult)
    tim: MetricResult = field(default_factory=MetricResult)
    final_score: float = 0.0
    final_class: str = "SIN_DATOS"
    final_confidence: float = 0.0
    confidence_tier: str = "LOW"
    allocation: float = 0.0
    raw_allocation: float = 0.0
    sector: str = "Unknown"
    warnings: List[str] = field(default_factory=list)
...
class ConfidenceEngine:
    """Calcula confianza para cada métrica basado en calidad de datos."""
    
    @staticmethod
    def calculate_confidence(metric_type: str, data_points: int, 
                            used_fallback: bool, sanity_passed: bool,
                            data_age_days: int = 0) -> float:
        """
        Calcula score de confianza (0.0 - 1.0)
        """
        base = 1.0
        
        # Penalización por uso de fallback
        if used_fallback:
            base *= 0.85
        
        # Penalización por pocos data points
        if data_points < 3:
            base *= 0.70
        elif data_points < 5:
            base *= 0.85
        
        # Penalización si falló sanity check
        if not sanity_passed:
            base *= 0.50
        
        # Penalización por datos viejos
        if data_age_days > 90:
            base *= 0.80
...
class DataValidator:
    ...
    @staticmethod
    def validate_class_consistency(score: float, class_: str) -> Tuple[bool, str]:
        """Valida que el score y la clase sean consistentes."""
        expected = "SIN_DATOS"
        if score >= 80:
            expected = "ALTA"
        elif score >= 60:
        ...
        if class_ != expected and score > 0:
            return False, f"Inconsistencia: score={score} debería ser {expected}, es {class_}"
        return True, ""
    
    @staticmethod
    def validate_price(price: float) -> Tuple[bool, float, str]:
        """Valida que el precio sea razonable."""
        if price is None or math.isnan(price) or price <= 0:
            return False, 0.0, "Precio inválido"
        if price > 100000:  # Sanity check
            return False, 0.0, f"Precio sospechoso: {price}"
        return True, price, ""
    
    @staticmethod
    def sanitize_ticker_data(data: Dict) -> Tuple[Dict, List[str]]:
        """Sanitiza datos de un ticker y retorna (datos_limpios, warnings)."""
        warnings = []
        clean = {}
        
        # Sanitizar scores
        for key in ["growthScore", "valueScore", "timingScore", "finalScore"]:
            if key in data:
                val = data[key]
                if val is None or math.isnan(val):
                    warnings.append(f"{key} es null/NaN, usando default 0")
                    clean[key] = 0.0
                else:
                    clean[key] = DataValidator.clamp_score(val)
        
        # Validar consistencia clase/score
        if "finalScore" in clean and "finalClass" in data:
            ok, msg = DataValidator.validate_class_consistency(
                clean["finalScore"], data["finalClass"]
            )
            if not ok:
                warnings.append(msg)
...
class MarketRegimeDetector:
    """Detecta el régimen de mercado actual usando SPY y VIX."""
    
    def __init__(self):
        self.spy = None
        self.vix = None
    
    def fetch_market_data(self) -> Tuple[Optional[float], Optional[float]]:
        """Obtiene datos de SPY y VIX."""
        try:
            spy = yf.Ticker("SPY")
            spy_hist = spy.history(period="250d")
            if spy_hist.empty:
                return None, None
            ...
            return current_spy, current_vix, spy_ma200
            
        except Exception as e:
            print(f"Error fetching market data: {e}")
            return None, None, None
    
    def detect_regime(self, spy_price: float, vix: float, spy_ma200: float) -> str:
        """
        Detecta régimen basado en VIX y tendencia SPY.
        """
        if vix is None or spy_price is None:
            return "neutral"
        
        # Panic: VIX > 25 + SPY debajo de MA200
        if vix > REGIME_THRESHOLDS["vix_panic"] and spy_price < spy_ma200:
            return "panic"
        
        # Bear: VIX > 20 o SPY debajo de MA200
        if vix > REGIME_THRESHOLDS["vix_bear"] or spy_price < spy_ma200:
            return "bear"
        
        # Bull: VIX < 12 y SPY arriba de MA200
        if vix < REGIME_THRESHOLDS["vix_bull"] and spy_price > spy_ma200:
            return "bull"
        
        return "neutral"
    
    def get_regime_weights(self, regime: str, base_strategy: str) -> Dict:
        """Retorna pesos ajustados según régimen."""
        if regime in REGIME_WEIGHTS:
            return REGIME_WEIGHTS[regime]
        return STRATEGY_CONFIG[base_strategy]["weights"]
...
class AllocationEngine:
    """Calcula allocations con límites y normalización."""
    
    def __init__(self, strategy: str):
        self.config = STRATEGY_CONFIG[strategy]
        self.max_pos = self.config["max_position"]
        self.max_sector = self.config["max_sector"]
        self.min_cash = self.config["min_cash"]
    
    def calculate_raw_allocation(self, final_score: float, confidence: float) -> float:
        """
        Allocation base basada en score y confianza.
        Retorna decimal 0-1 (ej: 0.05 = 5%).
        """
        if final_score <= 0:
            return 0.0
        
        # Usar determine_allocation para consistencia
        alloc = determine_allocation(final_score, "")
        return float(alloc) if alloc else 0.0
    def apply_position_cap(self, allocation: float) -> float:
        """Aplica límite máximo por posición."""
        return min(allocation, self.max_pos)

    def normalize_allocations(self, results: List) -> List:
        """
        Normaliza allocations para que sumen 1 - min_cash.
        Aplica caps por posición y sector.
        Acepta TickerResult o dict.
        """
        if not results:
            return results
        
        # Helper para leer/escribir en TickerResult o dict
        def get_val(r, key, default=0):
            if hasattr(r, key):
                return getattr(r, key)
            return r.get(key, default) if isinstance(r, dict) else default
        
        def set_val(r, key, value):
            if hasattr(r, key):
                setattr(r, key, value)
            elif isinstance(r, dict):
                r[key] = value
        
        # Paso 1: Aplicar cap por posición
        for r in results:
            raw = get_val(r, 'allocation', 0)
            set_val(r, 'raw_allocation', raw)
            capped = min(raw, self.max_pos)
            set_val(r, 'allocation', capped)
        
        # Paso 2: Verificar límites por sector
        sector_totals = {}
        for r in results:
            sector = get_val(r, 'sector', 'Unknown')
            sector_totals[sector] = sector_totals.get(sector, 0) + get_val(r, 'allocation', 0)
        
        for sector, total in sector_totals.items():
            if total > self.max_sector:
                factor = self.max_sector / total
                for r in results:
                    if get_val(r, 'sector', 'Unknown') == sector:
                        new_alloc = get_val(r, 'allocation', 0) * factor
                        set_val(r, 'allocation', round(new_alloc, 4))
        
        # Paso 3: Normalizar para que sumen 1 - min_cash
        total = sum(get_val(r, 'allocation', 0) for r in results)
        target = 1.0 - self.min_cash
        ...
        # Re-aplicar caps después de normalización
        for r in results:
            new_alloc = min(get_val(r, 'allocation', 0), self.max_pos)
            set_val(r, 'allocation', round(new_alloc, 4))
    ...
    def check_health(self, results: List) -> PortfolioHealth:
        """Verifica salud del portfolio. Acepta TickerResult o dict."""
        health = PortfolioHealth()
        
        # Helper para leer allocation de TickerResult o dict
        def get_alloc(r):
            if hasattr(r, 'allocation'):
                return r.allocation
            return r.get('allocation', 0) if isinstance(r, dict) else 0
        
        def get_sector(r):
            if hasattr(r, 'sector'):
                return r.sector
            return r.get('sector', 'Unknown') if isinstance(r, dict) else 'Unknown'
        
        total = sum(get_alloc(r) for r in results)
        health.total_allocation = round(total * 100, 2)
        health.cash_allocation = round((1.0 - total) * 100, 2)
        
        # Verificar total
        if total > 1.0:
            health.status = "ERROR"
            health.messages.append(f"Over-allocated: {health.total_allocation}% > 100%")
        elif total > 0.95:
            health.status = "WARNING"
            health.messages.append(f"Near full allocation: {health.total_allocation}%")
        
        # Verificar cash mínimo
        if health.cash_allocation < self.min_cash * 100:
            health.status = "WARNING"
            health.messages.append(f"Cash below minimum: {health.cash_allocation}% < {self.min_cash*100}%")
        
        # Sector breakdown
        sector_totals = {}
        for r in results:
            sector = get_sector(r)
            sector_totals[sector] = sector_totals.get(sector, 0) + get_alloc(r)
        
        health.sector_breakdown = {k: round(v * 100, 2) for k, v in sector_totals.items()}
        
        # Sector alerts
        for sector, alloc in health.sector_breakdown.items():
            if alloc > self.max_sector * 100:
                health.sector_alerts.append(f"⚠️ {sector}: {alloc}% > {self.max_sector*100}%")
                health.status = "ERROR"
            elif alloc > self.max_sector * 100 * 0.8:
                health.sector_alerts.append(f"⚡ {sector}: {alloc}% near limit")
                if health.status == "OK":
                    health.status = "WARNING"
...
class PerformanceTracker:
    """Registra decisiones y calcula métricas de performance."""
    
    def __init__(self, db: firestore.Client):
        self.db = db
    
    def log_conviction(self, ticker: str, final_score: float, 
                      confidence: float, allocation: float,
                      regime: str, strategy: str):
        """Registra una decisión de convicción."""
        log_ref = self.db.collection("conviction_log").document()
        log_ref.set({
            "ticker": ticker,
    ...
    def log_trade(self, ticker: str, action: str, shares: float,
                  price: float, conviction: float):
    ...
    def get_monthly_stats(self, month: str = None) -> Dict:
        ...
        # Este es un placeholder - en producción harías queries a Firestore
        return {
            "month": month,
            "totalDecisions": 0,
            "avgConviction": 0.0,
...
def determine_allocation(final_score: float, conviction: str) -> float:
    """Retorna allocation como decimal (0.05 = 5%)"""
    if "COMPRA AGRESIVA" in conviction: return 0.10
    if "PEQUEÑA POSICIÓN" in conviction: return 0.03
    if "DCA" in conviction: return 0.02
    if "WATCHLIST" in conviction: return 0.0
    if "SWING ONLY" in conviction: return 0.02
    if "ESPECULACIÓN" in conviction: return 0.01
    if "BUFFETT" in conviction: return 0.0
    
    if final_score >= 80: return 0.05
    elif final_score >= 65: return 0.03
    elif final_score >= 50: return 0.02
    elif final_score >= 35: return 0.01
    else: return 0.0
...
v7_ranges = {
    ...
    'Volatility_ATR': f'FK{START_ROW}:FK{END_ROW}',
        ...
        # ── NUEVO: Tier 1 columnas ──
    'BQ_Confidence': f'FL{START_ROW}:FL{END_ROW}',
    'VAL_Confidence': f'FM{START_ROW}:FM{END_ROW}',
    'TIM_Confidence': f'FN{START_ROW}:FN{END_ROW}',
    'Final_Confidence': f'FO{START_ROW}:FO{END_ROW}',
    'Confidence_Tier': f'FP{START_ROW}:FP{END_ROW}',
    'Market_Regime': f'FQ{START_ROW}:FQ{END_ROW}',
    'Regime_Adjusted': f'FR{START_ROW}:FR{END_ROW}',
    'Regime_Weights': f'FS{START_ROW}:FS{END_ROW}',
    'Raw_Allocation': f'FT{START_ROW}:FT{END_ROW}',
...
v7_defaults = {
    ...
    'BQ_Confidence': 0, 'VAL_Confidence': 0, 'TIM_Confidence': 0,
    'Final_Confidence': 0, 'Confidence_Tier': 'LOW',
    'Market_Regime': 'neutral', 'Regime_Adjusted': False,
    'Regime_Weights': 'BQ:0.50|VAL:0.30|TIM:0.20',
    'Raw_Allocation': 0,
}
...
def process_ticker_v7(symbol: str, strategy: str = "largo_plazo") -> Tuple[str, Dict[str, Any], List[str]]:
 ...
 # ═══════════════════════════════════════════════════════
    ...
    # ── NUEVO: Calcular confianza para cada métrica ──
    # Reemplazar las 3 líneas hardcodeadas con esto:
    bq_data_points = sum([
        1 if rev_cagr_3y is not None else 0,
        1 if eps_cagr_3y is not None else 0,
        1 if margin_stability is not None else 0,
        1 if fcf_consistency is not None else 0,
        1 if roic is not None else 0,
        1 if roe is not None else 0,
    ])
    bq_used_fallback = fmp_cb.is_open()  # FMP muerto = usamos fallback

    val_data_points = sum([
        1 if peg is not None else 0,
        1 if forward_pe is not None else 0,
        1 if ev_ebitda is not None else 0,
        1 if analyst_target is not None else 0,
        1 if fcf_yield > 0 else 0,
    ])
    val_used_fallback = fmp_cb.is_open()

    tim_data_points = sum([
        1 if sma_trend != "LATERAL" else 0,   # LATERAL = default = sin datos reales
        1 if vol_atr > 0 else 0,
        1 if vol_ratio != 1.0 else 0,
        1 if wr_current != 0 else 0,
    ])
    tim_used_fallback = len(hist) < 250 if hist is not None else True

    bq_confidence  = ConfidenceEngine.calculate_confidence("bq",  data_points=bq_data_points,  used_fallback=bq_used_fallback,  sanity_passed=True)
    val_confidence = ConfidenceEngine.calculate_confidence("val", data_points=val_data_points, used_fallback=val_used_fallback, sanity_passed=True)
    tim_confidence = ConfidenceEngine.calculate_confidence("tim", data_points=tim_data_points, used_fallback=tim_used_fallback, sanity_passed=True)
    
    # ── NUEVO: Detectar régimen de mercado ──
    regime_detector = MarketRegimeDetector()
    spy, vix, spy_ma200 = regime_detector.fetch_market_data()
    regime = regime_detector.detect_regime(spy, vix, spy_ma200) if spy else "neutral"
    regime_weights = REGIME_WEIGHTS.get(regime, REGIME_WEIGHTS["neutral"])
    
    # ── MODIFICADO: Usar pesos del régimen en vez de pesos fijos ──
    # Antes: w_bq, w_val, w_tim = strategy_weights[strategy]
    # Ahora:
    w_bq = regime_weights["bq"]
    w_val = regime_weights["val"]
    w_tim = regime_weights["tim"]
    ...
    final_score, bq_class, val_class, tim_class, entry_status, conviction = compute_final_score_v7(
        ...
        # ── NUEVO: Calcular confianza final y allocation ──
    final_confidence = (bq_confidence * w_bq + val_confidence * w_val + tim_confidence * w_tim)
    
    engine = AllocationEngine(strategy)
    raw_allocation = engine.calculate_raw_allocation(final_score, final_confidence)
    
    # ── NUEVO: Guardar campos Tier 1 en results ──
    results['BQ_Confidence'] = bq_confidence
    results['VAL_Confidence'] = val_confidence
    results['TIM_Confidence'] = tim_confidence
    results['Final_Confidence'] = final_confidence
    results['Confidence_Tier'] = "HIGH" if final_confidence >= 0.85 else "MEDIUM" if final_confidence >= 0.60 else "LOW"
    results['Market_Regime'] = regime
    results['Regime_Adjusted'] = True
    results['Regime_Weights'] = f"BQ:{w_bq}|VAL:{w_val}|TIM:{w_tim}"
    results['Raw_Allocation'] = determine_allocation(final_score, conviction)
    # results['Allocation'] se sobreescribe después de normalizar
    ...
    try:
        ...
        for i, val in enumerate(strategy_column):
            if val and val[0] and val[0].strip().lower() in ["largo_plazo", "swing", "crisis"]:
                ...
                # Detectar régimen de mercado global (una sola vez)
        regime_detector = MarketRegimeDetector()
        spy, vix, spy_ma200 = regime_detector.fetch_market_data()
        global_regime = regime_detector.detect_regime(spy, vix, spy_ma200) if spy else "neutral"
        logger.info(f"📊 Market Regime Detected: {global_regime.upper()} (VIX: {vix:.1f})" if vix else f"📊 Market Regime: {global_regime}")
        ...
        for idx in range(len(symbols)):
            ...
            for key in v7_ranges.keys():
                ...
                # ── NUEVO: Normalizar allocations con caps ──
        from dataclasses import dataclass  # si no está ya
        
        # ── FIX: No normalizar allocations ──
        # determine_allocation() ya retorna valores sensibles 0-1
        # La normalización anterior causaba allocations > 100%
        
        # Crear lista simple para health check
        ticker_results_list = []
        for idx in range(len(symbols)):
            if idx in ticker_results:
                _, res, _ = ticker_results[idx]
                alloc = res.get("Allocation", 0)
                if isinstance(alloc, str):
                    alloc = float(alloc.replace("%", "")) / 100 if "%" in alloc else float(alloc)
                ticker_results_list.append({
                    "ticker": symbols[idx],
                    "sector": res.get("Sector", "Unknown"),
                    "allocation": float(alloc) if alloc else 0,
                    "raw_allocation": float(alloc) if alloc else 0,
                    "final_score": res.get("Final_Score", 0),
                    "final_confidence": res.get("Final_Confidence", 0)
                })
        
        # Health check con allocations reales
        engine = AllocationEngine(strategy_map.get(0, "largo_plazo"))
        health = engine.check_health(ticker_results_list)

        # ── NUEVO: Guardar metadata Tier 1 ──
        all_results['Market_Regime'] = [[global_regime]]
        all_results['Health_Status'] = [[health.status]]
        all_results['Total_Allocation'] = [[health.total_allocation]]
        all_results['Cash_Allocation'] = [[health.cash_allocation]]
        ...
        try:
            from firebase_uploader_v7_2 import upload_v7_to_firestore
