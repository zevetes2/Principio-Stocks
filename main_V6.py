...
def write_to_sheets(worksheet, all_results: Dict[str, List], symbols: List[str]):
    """
    Actualización SELECTIVA: solo modifica las filas de los tickers procesados.
    Preserva datos de tickers no incluidos en esta ejecución.
    Añade timestamp de última actualización por ticker.
    """
    logger.info("--- Escribiendo datos en Google Sheets (modo selectivo) ---")
    
    # 1. Leer tickers EXISTENTES en la hoja (para mapear índices)
    try:
        existing_tickers_raw = worksheet.get(ticker_range)
        existing_tickers = [row[0].strip().upper() if row and row[0] else "" 
                           for row in existing_tickers_raw]
    except Exception as e:
        logger.error(f"Error leyendo tickers existentes: {e}")
        existing_tickers = []
    
    # 2. Construir mapa: ticker → número de fila en Sheets (1-based)
    # START_ROW=7 significa que el índice 0 de existing_tickers = fila 7
    ticker_to_row = {}
    for idx, t in enumerate(existing_tickers):
        if t:
            ticker_to_row[t] = START_ROW + idx  # fila real en Sheets
    
    # 3. Timestamp actual para esta ejecución
    now_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
    
    # 4. Para cada ticker procesado, determinar su fila objetivo
    updates_by_row = {}  # row_number → {column_letter: value}
    new_tickers = []     # tickers que no existen en la hoja
    
    for i, sym in enumerate(symbols):
        sym_upper = sym.strip().upper()
        
        if sym_upper in ticker_to_row:
            target_row = ticker_to_row[sym_upper]
        else:
            # Ticker nuevo: asignar siguiente fila disponible
            next_row = START_ROW + len(existing_tickers) + len(new_tickers)
            if next_row > END_ROW:
                logger.warning(f"⚠️ {sym}: No hay filas disponibles (max {END_ROW})")
                continue
            target_row = next_row
            new_tickers.append(sym_upper)
            ticker_to_row[sym_upper] = target_row  # para evitar duplicados si aparece 2 veces
        
        # Preparar datos de esta fila
        row_data = {}
        for metric in ranges.keys():
            if metric in all_results and i < len(all_results[metric]):
                val = all_results[metric][i][0] if all_results[metric][i] else None
                row_data[metric] = sanitize_for_sheets(val)
            else:
                row_data[metric] = defaults.get(metric)
        
        # Añadir timestamp
        row_data['Last Update'] = now_str
        
        updates_by_row[target_row] = row_data
    
    # 5. Convertir a formato batch_update por columnas (más eficiente)
    # Agrupar por métrica: cada métrica necesita una lista de [fila, valor]
    batch_updates = []
    ...
    for metric, range_str in ranges.items():
        ...
        for row_num, row_data in updates_by_row.items():
            val = row_data.get(metric, defaults.get(metric))
        ...
        # Si es actualización contigua de muchas filas, usar rango
        # Si es dispersa, hacer updates individuales
        if len(rows_to_update) == 1:
            # Una sola fila: update directo
            col_letter = range_str.split(':')[0][0]  # extraer letra de columna
            cell = f"{col_letter}{rows_to_update[0]}"
            batch_updates.append({
                'range': cell,
                'values': values_to_update
            })
        else:
            # Múltiples filas: ver si son contiguas
            min_row = min(rows_to_update)
            max_row = max(rows_to_update)
            
            # Construir array completo del rango (con valores existentes o None)
            # Para no borrar datos de otras filas, solo actualizamos celdas específicas
            # Usamos batch_update con múltiples rangos pequeños
            
            # Estrategia: una entrada por fila (más seguro, no borra vecinos)
            for row_num in rows_to_update:
                col_start = range_str.split(':')[0]
                col_end = range_str.split(':')[1]
                # Extraer solo la letra de columna (asume formato A7:A190)
                col_letter = ''.join(c for c in col_start if c.isalpha())
                cell = f"{col_letter}{row_num}"
                val = updates_by_row[row_num].get(metric, defaults.get(metric))
                batch_updates.append({
                    'range': cell,
                    'values': [[val]]
                })  
    ...
    if batch_updates:
        try:
            # gspread permite batch_update con múltiples rangos
            worksheet.batch_update(batch_updates, value_input_option='USER_ENTERED')
            logger.info(f"✅ {len(batch_updates)} celdas actualizadas ({len(updates_by_row)} tickers)")
            if new_tickers:
                logger.info(f"   🆕 Nuevos tickers añadidos: {new_tickers}")
        except Exception as e:
            logger.error(f"❌ Error en batch_update: {e}")
            # Fallback: individual
            for upd in batch_updates:
                try:
                    worksheet.update(range_name=upd['range'], values=upd['values'])
                    time.sleep(0.5)
                except Exception as e2:
                    logger.error(f"   ✗ Fallo en {upd['range']}: {e2}")
    else:
